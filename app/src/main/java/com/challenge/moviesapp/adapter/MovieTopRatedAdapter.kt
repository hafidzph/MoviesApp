...
class MovieTopRatedAdapter(private var listNowPlaying: List<ResultTopRated>): RecyclerView.Adapter<MovieTopRatedAdapter.ViewHolder>() {
    inner class ViewHolder(private var binding: MovieItemBinding): RecyclerView.ViewHolder(binding.root){
        @SuppressLint("SetTextI18n")
        fun bind(topRated: ResultTopRated){
            val imgUrl = "https://image.tmdb.org/t/p/w500/${topRated.posterPath}"
            val userScore = kotlin.math.round(topRated.voteAverage * 10).toInt()
            binding.apply {
                tvTitle.text = topRated.title
                tvDate.text = topRated.releaseDate
